  ...
  async function prepare(){
    ...
    try{
      ...
      const parsed=normalized.map(source=>{
        ...
        const priceText=cell(source,"price_eur").replace(",","."),price=Number(priceText);if(!priceText||!Number.isFinite(price)||price<0||price>999||Math.abs(price*100-Math.round(price*100))>1e-7)errors.push("Preis fehlt oder ist ungültig (0–999 €, höchstens zwei Nachkommastellen)");
